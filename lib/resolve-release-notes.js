...
const followingReleaseStartRe = new RegExp(
	`^#+ \\[(${ versionRegExpString })\\]\\(([^)]+)\\) \\((\\d{4}-\\d{2}-\\d{2})\\)$`, "u"
);
const initialReleaseStartRe = new RegExp(
	`^#+ (${ versionRegExpString }) \\((\\d{4}-\\d{2}-\\d{2})\\)$`, "u"
);
...
module.exports = changeLogPath => {
	...
	readStream.pipe(split()).on("data", line => {
		const matches = (() => {
			const followingReleaseMatches = line.match(followingReleaseStartRe);
			if (followingReleaseMatches) return followingReleaseMatches;
			const initialReleaseMatches = line.match(initialReleaseStartRe);
			if (!initialReleaseMatches) return null;
			initialReleaseMatches.splice(2, 0, null);
			return initialReleaseMatches;
		})();
		if (matches) {
